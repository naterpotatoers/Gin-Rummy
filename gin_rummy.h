...
int List<T>::find(T card){
	...
	if(firstPtr->data == card)
		return i;
	while(curr and result == -1){ //run thru whole list cause dont want no seg fault
		if(curr->data == card)
			result = i;
		else{
			curr = curr->next;
...
T List<T>::remove_at(int i){
	T result;
//	if(firstPtr == nullptr)
//		return result;//wont accept return 0; since its and int :(
...
enum Val{START, ACE, TWO, THREE, FOUR, FIVE, SIX, SEVEN, EIGHT, NINE, TEN, JACK, QUEEN, KING};
