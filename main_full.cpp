...
void whoseTurn()
{ //couts simple statement, waits for user response
	for (int i = 0; i < 60; i++)
	{
		cout << endl;
	}
...
{ //
	if (boolTakenFromDiscard)
		cout << "Player " << !turnSwap << " took the " << takenFromDiscard << " from the discard pile and discarded the " << discard.peek() << "." << endl
			 << endl;
	else if (!boolTakenFromDiscard)
		cout << "Player " << !turnSwap << " took a card from stock and discarded " << discard.peek() << "." << endl
			 << endl;
	...
	cout << "Take Discard\t[U] " << endl
		 << endl; //Take top of discard
...
void execute(List<Card> *hands, Queue<Card> &deck, Stack<Card> &discard, bool &wasDiscarded)
{
	...
	{ //recursively call execute if invalid input, might need to change to prevent stack overflow...
...
void discardDecision(List<Card> *hands, Stack<Card> &discard, bool &wasDiscarded)
{
	...
	while (indexNum == -1)
	{
		...
		{ //if player tries to discard card just pulled from discard pile
			cout << "Invalid Input - Cannot Discard Card From Discard." << endl;
			indexNum = -1;
		}
		else if (indexNum == -1)
		{ //player attempts invalid input
...
void checkWinCondition(List<Card> *hands, bool &winCondition)
{
	...
	while (i < 10)
	{												 //translates the card value into bitset values
	...
	cout << "The Binary Representation of Player Hand by Suit & Combined" << endl
		 << "s: " << s << endl
		 << "d: " << d << endl
		 << "c: " << c << endl
		 << "h: " << h << endl
	...
	cout << "Sum Of Cards: " << sumOfCards << endl
		 << endl;
	cout << endl
		 << "The Binary Representation of Player Hand by Suit & Combined" << endl
		 << "s: " << s << endl
		 << "d: " << d << endl
		 << "c: " << c << endl
		 << "h: " << h << endl
